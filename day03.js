...
function closestIntersection(pathA, pathB) {
  var intersectingPoints = intersection(points(pathA), points(pathB));
  if (intersectingPoints.size < 1) {
    return "error, no intersection";
  }
  return [...intersectingPoints].reduce(function (prev, curr) {
    var curDist = Number.parseInt(curr.split()[0]);
    return Math.min(prev, curDist);
  }, Number.MAX_VALUE);
}

console.assert(closestIntersection("R8,U5,L5,D3", "U7,R6,D4,L4") === 6);
