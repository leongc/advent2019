...
function countOrbits(map) {
  let result = 0;
  let depth = 0;
  let currentQueue = ['COM'];
  while (currentQueue.length > 0) {
    let nextQueue = [];
    for (let o of currentQueue) {
      result += depth;
      if (map.has(o)) {
        let children = map.get(o);
...
console.assert(countOrbits(orbitalMap(example)) === 42);
